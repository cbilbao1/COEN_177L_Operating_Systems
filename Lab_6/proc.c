 ...
 */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	int chance = rand() % 15;	/* This variable handles the random chance, 1/15th chance for randomness */
	if(q > 3 && chance == 1) {	/* If q is greater than 3, 1/15th chance for chance to have value 1 */
		q = (rand() % (NR_SCHED_QUEUES - 4)) + 4;	/* Reassign q to random lower priority job q>=4 */
	}
	if(!(rp = rdy_head[q])) {
